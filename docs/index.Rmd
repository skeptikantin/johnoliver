...
The data is only from the *main segments* of each episode, which range in duration from 6 to 34 minutes. Until last year, those were the only dose of John available to most the outside world without a TV or cable subscription. (I fetched all data in late 2024.)
...
For the graph above, I used the simplest possible method, measuring speed in words per minute. `NA` in a row indicates that the main segments or auto-generated subtitles were not available on YouTube.
...
Gentle works reasonably well on audio files longer than a few seconds. This comes at the cost of accuracy, but patterns should be clear enough for over 370,000 data points. Gentle also returns all time stamps in centiseconds (1/100th of a second), whereas small-data "proper phonetics" usually works in milliseconds.

The package {nsyllables} counts the syllables per word to give us syllables per second of that particular word. Pauses are calculated as the difference of the end of one word and the beginning of the next, and are `NA` where the adjacent word could not be aligned.
...
* **Recognised** indicates if Gentle was successful in locating the word in the audio file.
* Where **NA**, that's counted as an alignment error — no further metrics can be determined.
* **Syllables per second** is a word-level metric — number of syllables divided by the word duration.
* The main segments on YouTube generally begin with a shortened intro of 4s — this artifact will probably be of negligible effect in the grander scheme of things.

It is worth noting that (longer) pauses between words generally occur (i) at phrase boundaries, like the "spoken" comma you'd expect after *specifically*, or (ii) for dramatic effect, like before *land* — both are suitably [clearly audible in segment](https://youtu.be/_-0J49_9lwc?si=jkoQM6E-U-Rec9mZ).

---

If we now average speed and pauses over all the words in each segment, we get a visual confirmation that that John's speech — and arguably the show's pace in general — have become much faster.

**Over time, words are spoken more quickly and there are shorter pauses between the words:**
...
Of course in faster-paced speech, pauses between words also get shorter. That being said — i
In connected speech, pauses between words is usually 0 at any speech level, especially at the phrase level. Therefore, an average decrease from 120ms to 90ms does suggest John is concatenating phrases and sentences more quickly *overall*, adding to the big-picture impression of a faster pace.
...
But for the sake of illustration, let's include a few additional predictors. They also explain why some files were excluded for the analysis:
...
2. Alignment errors (`align errors`) and pauses (`pause_mean`) are strongly correlated. This is almost entirely due to a handful of files with extreme alignment errors — leading to 30s+ pauses. We remove(d) segments with an implausibly high average pause of over 200ms. It also makes these two variables statistically less suitable. 
3. The number of an episode within a season is not correlated with any other variable. We use this variable to "test" if speech rate is associated with a kind of "training" or "fluency" effect across a season. Conceptually speaking, there is no reason to believe that this should be a factor (cuz, why?).
...
callout("On average, with every <strong>season</strong>, John's speaking rate increased by <strong>2.3 words per minute</strong>. Since we have 11 seasons, that is a rather intuitive statistical confirmation of increase from <strong>150</strong> to almost <strong>180 wpm</strong> which we eye-balled off the scatterplot.")
```
...
The actual numbers — 25 wpm more in 2014 than in 2025 — need to be taken with a grain of salt. Let's not forget that this average is measured across an entire segment, including intros/outros, audience reactions, pauses for dramatic effect, or the segments of other people. In general, we assume that this 'noise' is relatively constant across segments (or episodes, probably), but many of the mentioned factors probably 'drag' the average down a bit when compared to the speaking rates of other people or contexts.

As for the reasons why we see John's pattern — pure speculation, and not my area of expertise at all.

An natural reaction is that John is indeed also *angrier* — we intuitively associate emotional speech with greater speed. And there is after all a significant amount of stuff that's been getting progressively worse since 2014.
...
A topic-modelling analysis might be interesting to answer whether more political segment topics (but how define?) associated with more emotional speech styles. The obvious issue: all of John's stories are somewhat political and arguably equally enraging. Exploring the patterns — see the interactive plot below — doesn't seem to suggest that it has anything to do with the topic. But that'd be for another day.
